...
export default function Distributors() {
  ...
  const { data: distributors = [], isLoading: distributorsLoading } = useQuery<Distributor[]>({
    queryKey: ["api", "partners", "distributors", statusFilter],
    enabled: !!user && !isLoading,
    retry: 3,
  });

  const filteredDistributors = distributors.filter(distributor => {
    const matchesSearch = distributor.business_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Manage your distribution network</p>
        </div>
          ...
          <CardContent>
            {distributorsLoading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">No distributors match your current filters.</p>
              </div>
            ) : (
                      ...
                      <tr key={distributor.id} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-medium text-gray-900">{distributor.business_name}</div>
                        ...
                        <td className="py-4 px-4">
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2 text-sm">
                              ...
                              <span className="text-gray-900">{distributor.phone_number}</span>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-4">
                          <div className="font-medium text-gray-900">{distributor.total_products}</div>
                          <div className="text-xs text-gray-500">products</div>
                        </td>
                        <td className="py-4 px-4">
                          <div className="font-medium text-gray-900">{distributor.total_orders}</div>
